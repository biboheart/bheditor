...
export default class Input extends Module {
    ...
    public nodes: { [key: string]: HTMLElement } = {
        input: $.make('div', 'editor-hidden-input', 'left: 0; top: 0;', {
            spellcheck: false,
            tabIndex: '-1',
    ...
    private pasteHandler(event: ClipboardEvent) {
        ...
        if (paste) {
            /* const paragraph: Paragraph|null = this.editorModules.PageManager.current.item ? this.editorModules.PageManager.current.item.getParagraph() : null;
            if (paragraph) {
                paragraph.insert(paste);
            } */
    ...
    private readFromDOMSoon(event: InputEvent|CompositionEvent) {
        ...
        this.readDOMTimeout = setTimeout(() => {
            ...
            if (event.data) {
                /* const paragraph: Paragraph|null = this.editorModules.PageManager.current.item ? this.editorModules.PageManager.current.item.getParagraph() : null;
                if (paragraph) {
                    paragraph.insert(event.data as string);
                } */
            }
            this.nodes.input.innerText = '';
        }, 80)
